...
class GetDepth:
    def __init__(self):
        ...
        self.pu = None
        self.py = None
        self.sub_c = rospy.Subscriber("/aim_px", Detection2DArray, self.callback_center, queue_size=1) # /yolov7/yolov7 or /aim_px
        ...
        self.sub_odom = rospy.Subscriber("/mavros/local_position/odom", Odometry, self.callback_odom, # /visual_slam/odom or /mavros/local_position/odom
                                      queue_size=1)
        self.pub_depth = rospy.Publisher("obj_depth", Float64, queue_size=1)
        self.pub_position = rospy.Publisher("obj_position", PoseStamped,queue_size=1)
    ...
    def callback_depth(self,data=Image):
        ...
        try:
            ...
            if self.puv_refreshed == True:# & self.odom_refreshed == True:
                ...
                position_list = [self.body_pos_x,self.body_pos_y, self.body_pos_z]
                position_matrix = np.zeros((3,1))
                for i in range(3):
                    position_matrix[i] = position_list[i]
                p =  np.dot(rotation_matrix,position_matrix) + position_matrix
                self.obj_x,self.obj_y,self.obj_z = p[0:3]
                # print('self.obj_xyz:\n',self.obj_x,self.obj_y,self.obj_z,'\n')

                # Generate obj position msg
                obj_positon_msg = PoseStamped()
                obj_positon_msg.pose.position.x = self.obj_x
                obj_positon_msg.pose.position.y = self.obj_y
                obj_positon_msg.pose.position.z = self.obj_z #!!!!!
                obj_positon_msg.pose.orientation.x = 0
                obj_positon_msg.pose.orientation.y = 0
                obj_positon_msg.pose.orientation.z = 0
                obj_positon_msg.pose.orientation.w = 1

                # publish obj_position
                self.pub_position.publish(obj_positon_msg)
    ...
    def callback_center(self, data=Detection2DArray):
        self.pu = data.detections[0].bbox.center.x
        self.pv = data.detections[0].bbox.center.y
